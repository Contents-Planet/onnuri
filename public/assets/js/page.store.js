...
var Page = {
  ...
  /* 현재위치 ㅎ*/
  CurrentLocation : function(_callback) {
    if (navigator.geolocation) {
      // GeoLocation을 이용해서 접속 위치를 얻어옵니다
      navigator.geolocation.getCurrentPosition(function(position) {
        var lat = position.coords.latitude, // 위도
          lon = position.coords.longitude // 경도
        nowLocation = {"lat" : lat, "lon" : lon}
  ...
  /* 지도 생성 */
  RenderMap : function() {
    Page.CurrentLocation(function(res) {
  ...
  AddMarker : function(data){
    ...
    Page.SearchAddrFromCoords(center, function(res){
      ...
      $.each($addData, function(index, row){
          ...
          Page.SearchCordsFromAdd(row.address, function(coords) {
